...
setTimeout(function(){	grav();	init();	}, 1000);
...
function init() {
	...
	addEvent(document, 'touchstart', onDocumentTouchStart);
	addEvent(document, 'touchmove', onDocumentTouchMove);
	addEvent(document, 'touchend', onDocumentTouchEnd);

	addEvent(window, 'deviceorientation', onWindowDeviceOrientation);
	...
	//for (var i = 0; i < walls.length; i++) {
		...
		//wall.className = 'wall';
		//wall.style.border = '1px solid #999';
		//wall.style.background = 'white';
		//wall.style.left = walls[i].m_position0.x - (stage[2] >> 1) + 'px';
		//wall.style.top = walls[i].m_position0.y - (stage[3] >> 1) + 'px';
		//wall.style.height = stage[3];
		//wall.style.width = stage[2];
	...
	for ( var i = 0; i < elements.length; i ++ ) {
		...
		addEvent(element, 'mousedown', onElementMouseDown);
		addEvent(element, 'mouseup', onElementMouseUp);
		addEvent(element, 'click', onElementClick);
...
function addToGame(that) {
	if (that.children().length < 1) {
		that.addClass("box2d");
		that.css({
			"zIndex": "99",
			"outline": "2px dotted red",	// show the bounding box
			"textIndent": "0"				// for baidu.com
		});
	}
}
...
function grav(){
	...
	for (var i = 0, ele_type; ele_type = targetClass[i]; i++){
		$(ele_type).each(function () {
			addToGame($(this));
		});
	}	
}
